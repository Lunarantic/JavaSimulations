...
public class Sim2 {
	public static void main(String[] args) {
		...
		for (double b = 0.025; processes.size() < 10; processes.add(new Process("P"+processes.size(), 0.0, RandomGenerator.random() * 120 + 120, 0.06, b += 0.005)));
		FileUtility.writeTotFile("Sim2.log", "Shortest Job First Scheduler with alpha = 1" + "\n");
		FileUtility.writeTotFile("Sim2.log", (new ShortestJobFirstScheduler()).execute(processes) + "\n");
		processes = new ArrayList<>();
		RandomGenerator.reset();
		for (double b = 0.025; processes.size() < 10; processes.add(new Process("P"+processes.size(), 0.0, RandomGenerator.random() * 120 + 120, 0.06, b += 0.005)));
		FileUtility.writeTotFile("Sim2.log", "Shortest Job First Scheduler with alpha = 1/2" + "\n");
		FileUtility.writeTotFile("Sim2.log", (new ShortestJobFirstScheduler(0.0, 1/2.0)).execute(processes) + "\n");
		processes = new ArrayList<>();
		RandomGenerator.reset();
		for (double b = 0.025; processes.size() < 10; processes.add(new Process("P"+processes.size(), 0.0, RandomGenerator.random() * 120 + 120, 0.06, b += 0.005)));
		FileUtility.writeTotFile("Sim2.log", "Shortest Job First Scheduler with alpha = 1/3" + "\n");
		FileUtility.writeTotFile("Sim2.log", (new ShortestJobFirstScheduler(0.0, 1/3.0)).execute(processes) + "\n");
		processes = new ArrayList<>();
		RandomGenerator.reset();
		for (double b = 0.025; processes.size() < 10; processes.add(new Process("P"+processes.size(), 0.0, RandomGenerator.random() * 120 + 120, 0.06, b += 0.005)));
